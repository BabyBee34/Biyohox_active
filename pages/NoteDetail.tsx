...
import { ArrowLeft, Calendar, Eye, Share2, Printer, Bookmark, ChevronRight, FolderOpen, Clock } from 'lucide-react';
...
const NoteDetail: React.FC = () => {
  ...
  const note = MOCK_RESOURCES.find(r => r.id === id);
  
  // Mock related notes (same grade)
  const relatedNotes = MOCK_RESOURCES
    .filter(r => r.grade === note?.grade && r.id !== note?.id && r.type === 'note')
    .slice(0, 3);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
                ...
                <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-6 text-white text-center relative overflow-hidden">
                    ...
                    <button className="w-full py-2 bg-bio-mint text-slate-900 font-bold rounded-lg hover:bg-white transition-colors text-sm relative z-10">
                        PDF Arşivine Git
                    </button>
                </div>
